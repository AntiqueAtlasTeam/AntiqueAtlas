...
public enum ExportProgressOverlay {
	...
	public void draw(MatrixStack matrices, int scaledWidth, int scaledHeight, float partial) {
		...
		font.draw(matrices, l.header, ( x )*s -headerWidth/2, ( y )*s - 14, 0xffffff);
		int statusWidth = font.getWidth(l.status);
		font.draw(matrices, l.status, ( x )*s -statusWidth/2, ( y )*s, 0xffffff);
