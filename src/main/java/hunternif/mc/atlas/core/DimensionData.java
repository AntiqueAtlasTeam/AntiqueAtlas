...
public class DimensionData implements ITileStorage {
	 ...
	 * 
	 * Key is a IntVec2 representing the gilegroup's position in units of TileGroup.CHUNK_STEP
	 */
	private final Map<IntVec2, TileGroup> tileGroups = new ConcurrentHashMap<IntVec2, TileGroup>(2, 0.75f, 2);
	 ...
	 */
	public Map<IntVec2, Tile> getSeenChunks() {
		Map<IntVec2, Tile> chunks = new ConcurrentHashMap<IntVec2, Tile>(2, 0.75f, 2);
		Tile t = null;
		for (Map.Entry<IntVec2, TileGroup> entry: tileGroups.entrySet()){
	...
	public void setTile(int x, int y, Tile tile) {
		IntVec2 groupPos = getKey().set((int)Math.floor(x / (float) TileGroup.CHUNK_STEP),
	...
	public void putTileGroup(TileGroup t){
		IntVec2 key = new IntVec2(Math.floorDiv(t.scope.minX,TileGroup.CHUNK_STEP), Math.floorDiv(t.scope.minY,TileGroup.CHUNK_STEP));
	...
	public void addData(DimensionData other){
		for (Entry<IntVec2, TileGroup> e:other.tileGroups.entrySet()){
	...
	public NBTTagList writeToNBT() {
		...
		for (Entry<IntVec2, TileGroup> entry : tileGroups.entrySet()) {
	...
	public void syncOnPlayer(int atlasID, EntityPlayer player){
		...
		int count = 0;
		int total = 0;
		for (Entry<IntVec2, TileGroup> t:tileGroups.entrySet()){
			tgs.add(t.getValue());
			count++;
			total++;
	...
	public boolean equals(Object obj) {
		...
		for (IntVec2 entry : tileGroups.keySet()) {
