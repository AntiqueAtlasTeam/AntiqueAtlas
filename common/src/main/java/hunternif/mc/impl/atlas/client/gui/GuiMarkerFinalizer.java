...
public class GuiMarkerFinalizer extends GuiComponent {
    ...
    public void init() {
        ...
        addDrawableChild(btnCancel = ButtonWidget.builder(Text.literal("gui.cancel"), (button) -> {
            closeChild();
        }).dimensions(this.width / 2 + BUTTON_SPACING / 2, this.height / 2 + 40, BUTTON_WIDTH, 20).build());
        textField = new TextFieldWidget(MinecraftClient.getInstance().textRenderer, (this.width - 200) / 2, this.height / 2 - 81, 200, 20, Text.literal("gui.antiqueatlas.marker.label"));
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float partialTick) {
        ...
        drawCentered(drawContext, Text.translatable("gui.antiqueatlas.marker.label"), this.height / 2 - 97, 0xffffff, true);
        textField.render(drawContext, mouseX, mouseY, partialTick);
        drawCentered(drawContext, Text.translatable("gui.antiqueatlas.marker.type"), this.height / 2 - 44, 0xffffff, true);

        // Darker background for marker type selector
        drawContext.fillGradient(scroller.getGuiX() - TYPE_BG_FRAME, scroller.getGuiY() - TYPE_BG_FRAME,
