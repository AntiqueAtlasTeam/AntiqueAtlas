...
public class ExportImageUtil {
    ...
    public static void exportPngImage(WorldData biomeData, DimensionMarkersData globalMarkers,
        ...
        try {
            InputStream is = MinecraftClient.getInstance().getResourceManager().getResource(Textures.EXPORTED_BG).get().getInputStream();
            bg = ImageIO.read(is);
            is.close();
...
//					allTextures.add(type.getIcon());
            ...
            for (Identifier texture : allTextures) {
                try {
                    is = MinecraftClient.getInstance().getResourceManager().getResource(texture).get().getInputStream();
